...
count < 0 : 从表尾开始向表头搜素，移除与 value 相等的元素，数量为 count 的绝对值
...
// Ltrim 对一个列表进行修剪(trim)，就是说，让列表只保留指定区间的元素，不在指定区间之内的元素都将被删除
/**
...
// Hget 返回指定哈希集中该字段所关联的值，当字段不存在或者 key 不存在时返回nil
func (rc *Conn) Hget(key, filed string) (string, error) {
	return rc.client.HGet(key, filed).Result()
}

// Hmget 返回哈希表中，一个或多个给定域的值
/**
如果给定的域不存在于哈希表，那么返回一个 nil 值。
因为不存在的 key 被当作一个空哈希表来处理，所以对一个不存在的 key 进行 HMGet 操作将返回一个只带有 nil 值的表。
*/
func (rc *Conn) Hmget(key string, field ...string) ([]string, error) {
	result, err := rc.client.HMGet(key, field...).Result()
	if errors.Is(err, rd.Nil) {
		return nil, nil
	}
	if err != nil {
...
// ZAdd 将一个或多个 member 元素及其 score 值加入到zset中
/**
格式 value score value score ...
...
// Incr 用于将键的整数值递增1.如果键不存在，则在执行操作之前将其设置为0
/**
...
连接时将被 BLPOP 命令阻塞。当给定多个 key 参数时，按参数 key 的先后顺序依次检查各个列表，弹出第一个非空列表的头元素
*/
